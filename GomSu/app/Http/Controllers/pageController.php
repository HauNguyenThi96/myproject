...
class PageController extends Controller
{
    ...
    //     $product_promotion = Product::where('promotion_price','>',0)->limit(4)->get();
    //     return view('emails.email', compact('product_promotion'));
    // }
    ...
    public function postCheckout(Request $req){
        ...
        $data=array('name'=>$req->name, 'email'=>$req->email, 'address'=>$req->address, 'phone_number'=>$req->phone,'total'=>$cart->totalPrice, 'id_product'=>$key,'name'=>$cart->name, 'quantity'=>$value['qty'],'unit_price'=>($value['price']*$value['qty']), 'payment'=>$req->payment_method);
        Mail::send('emails.buyer', $data, function($message)
        {
            $message->to($this->email, $this->name, $this->address, $this->phone_number,$this->total, $this->payment, $this->id_product,$this->name, $this->quantity, $this->unit_price)->subject('Chào mừng bạn đến với Gốm Sứ THANK');
        });
        Session::forget('cart');    
